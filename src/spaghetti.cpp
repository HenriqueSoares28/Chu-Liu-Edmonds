...
int main() {
    std::ofstream outfile("spaghettiLog.csv"); // Cria e abre um arquivo CSV
    ...
    for (int q = 0; q < qt; ++q) {
        srand(q);
        clock_t start_time = clock(); // Inicia a medição do tempo
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < i; ++j) {
                if (rand() % 5 == 0)
                    solver.add_edge(i, j, 1 + rand() % wMax);
                if (rand() % 5 == 0)
                    solver.add_edge(j, i, 1 + rand() % wMax);
            }
        }

        solver.make_graph(); // Prepara o grafo para o algoritmo
        double a = solver.solve(0); // Executa o algoritmo

        // Calcula o tempo decorrido e imprime no arquivo CSV
        double elapsed = (clock() - start_time) / (double)CLOCKS_PER_SEC;
        outfile << q << "," << n << "," << a << "," << elapsed << "\n";
        
    }

    outfile.close(); // Fecha o arquivo CSV
    cout << "*** NO PROBLEM ***" << endl; // Indica que a execução foi concluída sem problemas
    return 0;
}